...
public class Message implements Serializable {
	...
	public Boolean isActMessage() {
		Optional<BaseMsgElement> findFirst = Arrays.stream(getMsgBody()).filter(e -> e.isActElement()).findFirst();
		return findFirst.isPresent();
	}
	
	@JsonIgnore
	public CustomContent getActCustomContent() {
		CustomContent customContent = Arrays.stream(getMsgBody()).filter(e -> e.isActElement())
		.map(e -> (CustomContent)e.getMsgContent())
		.findFirst().get();
