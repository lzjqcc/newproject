angular.module('app').service('imService',  function ($rootScope) {
        ...
        this.logout = function () {
            webim.logout(
                    function (resp) {
                        $rootScope.loginInfo.identifier = '未登录';
                        $rootScope.loginInfo.userSig = '';
                    }
            );
        }
        ...
        var initRecentContactList = function (cbOK, cbErr) {
            ...
            webim.getRecentContactList(
                    ...
                    function (resp) {
                        ...
                        $rootScope.Sessions = resp.Sessions;
                        //清空聊天对象列表
                        var sessList = document.getElementsByClassName("sesslist")[0];
                        sessList.innerHTML = "";
                        if (resp.Sessions && resp.Sessions.length > 0) {//如果存在最近会话记录
...
//初始化（创建群之前）选择好友表格
        function initSelectMyFriendGroupTable(data) {
            ...
            webim.Log.info('initSelectMyFriendGroupTable');
