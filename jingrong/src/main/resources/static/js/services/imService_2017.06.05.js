angular.module('app').service('imService', function ($rootScope) {
    ...
    this.sendGroupMsg = function (toAccount, msgtosend) {
...
//                        }
    ...
    //选择最后聊天的session，参数为 c2c/group和对像id
    this.selectSession = function (sess_type, to_id) {
        onSelSess(sess_type, to_id);
    }

    //读取群组成员
    this.getGroupMemberInfoById = function (group_id) {
    ...
    function addMsg(msg, prepend) {
        var tempObj = msg.sess._impl;
        if (!$rootScope.Sessions[tempObj.id]) {
            $rootScope.Sessions[tempObj.id] = {};
        }
...
//                    MsgRandom: item.MsgRandom, //消息随机数
        ...
        $rootScope.Sessions[tempObj.id].SessionId = tempObj.id;
        $rootScope.Sessions[tempObj.id].curMaxMsgSeq = tempObj.curMaxMsgSeq;
        $rootScope.Sessions[tempObj.id].SessionImage = tempObj.icon;
        $rootScope.Sessions[tempObj.id].isAutoRead = tempObj.isAutoRead;
        $rootScope.Sessions[tempObj.id].isFinished = tempObj.isFinished;
        $rootScope.Sessions[tempObj.id].SessionNick = tempObj.name;
        $rootScope.Sessions[tempObj.id].skey = tempObj.skey;
        $rootScope.Sessions[tempObj.id].MsgTimeStamp = tempObj.time;
        $rootScope.Sessions[tempObj.id].SessionType = tempObj.type;
        $rootScope.Sessions[tempObj.id].UnreadMsgCount = tempObj.unread;

        if (typeof $rootScope.Sessions[tempObj.id].msgs === 'undefined') {
            $rootScope.Sessions[tempObj.id].msgs = {};
        }
        angular.forEach(tempObj.msgs, function (x, index, array) {
            if (!$rootScope.Sessions[tempObj.id].msgs[x.seq]) {

//                            angular.forEach(x.elems, function (y, index, array) {
//                                if (y.type == 'TIMFaceElem') {
//                                    var data = y.content.getData();
//                                    var index = webim.EmotionDataIndexs[data];
//                                    var emotion = webim.Emotions[index];
//                                    
//                                    y.content.emotionUrl = '';
//                                    if (emotion && emotion[1]) {
//                                       y.content.emotionUrl = emotion[1];
//                                    }
//                                    x.elems[index] = y;
//                                }
//                            });

                $rootScope.Sessions[tempObj.id].msgs[x.seq] = x;
            }
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].seq = x.seq;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].fromAccount = x.fromAccount;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].isSend = x.isSend;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].random = x.random;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].time = x.time;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].subType = x.subType;
//                        $rootScope.Sessions[tempObj.id].msgs[x.seq].elems = x.elems;

//                        angular.forEach(x.elems, function (y, index, array) {
//
//                            var msg_obj = {};
//                            msg_obj.type = y.type;
//                            msg_obj.content = y.content;
//                            $rootScope.Sessions[tempObj.id].msgs[x.seq].msgbody.push(msg_obj);
//
//                        });
...
//搜索用户菜单点击事件
    function searchProfileByUserIdClick() {
        ...
        webim.Log.info('searchProfileByUserIdClick');
...
//初始化搜索用户表格
    function initSearchProfileTable(data) {
        webim.Log.info('initSearchProfileTable');
    ...
    var searchProfileByUserId = function () {
        ...
        webim.getProfilePortrait(
                ...
                function (resp) {
                    ...
                    webim.Log.info('searchProfileByUserId');
...
//初始化我的好友表格
    function initGetMyFriendTable(data) {
        webim.Log.info('initGetMyFriendTable');
                ...
                function (resp) {
                    if (resp.FriendNum > 0) {
                        ...
                        for (var i = 0; i < count; i++) {
                            ...
                            $rootScope.friends[friend_account] = {
...
//初始化我的好友申请表格
    function initGetPendencyTable(data) {
        webim.Log.info('initGetPendencyTable');
...
//初始化我的黑名单表格
    function initGetBlackListTable(data) {
        ...
        webim.Log.info('initGetBlackListTable');
    ...
    var deleteBlackList = function (del_account) {
        ...
        webim.deleteBlackList(
                ...
                function (resp) {
                    ...
                    webim.Log.info('deleteBlackList');
    ...
    var getRecentContactList = function () {
        ...
        webim.getRecentContactList(
                ...
                function (resp) {
                    ...
                    webim.Log.info('getRecentContactList');
...
//                                    tempSess = recentSessMap[firstSessId];//选中第一个会话
...
//搜索群(按ID)菜单单击事件
    function searchGroupMenuClick() {
        ...
        webim.Log.info('searchGroupMenuClick');
...
//搜索群(按名称)菜单单击事件
    function searchGroupByNameMenuClick() {
        ...
        webim.Log.info('searchGroupByNameMenuClick');
...
//初始化搜索群(按ID)结果表格
    function initSearchGroupTable(data) {
        ...
        webim.Log.info('initSearchGroupTable');
...
//初始化搜索群（按名称）结果表格
    function initSearchGroupByNameTable(data) {
        ...
        webim.Log.info('initSearchGroupByNameTable');
...
//初始化我的群组表格
    function initGetMyGroupTable(data) {
        ...
        webim.Log.info('initGetMyGroupTable');
...
//初始化邀请好友加群表格
    function initGetMyFriendGroupTable(data) {
        ...
        webim.Log.info('initGetMyFriendGroupTable');
    ...
    var getGroupPublicInfo = function () {
        ...
        webim.getGroupPublicInfo(
                ...
                function (resp) {
                    ...
                    webim.Log.info('getGroupPublicInfo');
    ...
    var searchGroupByName = function () {
        ...
        webim.searchGroupByName(
                ...
                function (resp) {
                    ...
                    webim.Log.info('searchGroupByName');
    ...
    var modifyGroup = function () {
        ...
        webim.modifyGroupBaseInfo(
                ...
                function (resp) {
                    ...
                    webim.Log.info('modifyGroupBaseInfo');
    ...
    var changeGroupOwner = function () {
        ...
        webim.changeGroupOwner(
                ...
                function (resp) {
                    ...
                    webim.Log.info('changeGroupOwner');
    ...
    var getMyGroup = function () {
        ...
        webim.getJoinedGroupListHigh(
                ...
                function (resp) {
                    ...
                    //webim.Log.info('getJoinedGroupListHigh');
        ...
        webim.getAllFriend(
                ...
                function (resp) {
                    ...
                    if (resp.FriendNum > 0) {
                        ...
                        webim.Log.info('getMyFriendGroup');
        ...
        webim.getJoinedGroupListHigh(
                ...
                function (resp) {
                    if (resp.GroupIdList && resp.GroupIdList.length) {
                        for (var i = 0; i < resp.GroupIdList.length; i++) {
                            var item = resp.GroupIdList[i];
                            $rootScope.groups[item.GroupId] = item;
...
//初始化群组成员表格
    function initGetGroupMemberTable(data) {
        ...
        webim.Log.info('initGetGroupMemberTable');
        ...
        webim.getGroupMemberInfo(
                ...
                function (resp) {
                    ...
                    for (var i in resp.MemberList) {
                        ...
                        if(typeof $rootScope.groups[group_id].MemberList == 'undefined'){
                            $rootScope.groups[group_id].MemberList  = [];
                        }
                        $rootScope.groups[group_id].MemberList.push(data);
                    }

                    webim.Log.info('getGroupMemberInfo');
    ...
    var modifyGroupMemberRole = function () {
        ...
        webim.modifyGroupMember(
                ...
                function (resp) {
                    ...
                    webim.Log.info('modifyGroupMemberRole');
    ...
    var modifyGroupMsgFlag = function () {
        ...
        webim.modifyGroupMember(
                ...
                function (resp) {
                    ...
                    webim.Log.info('modifyGroupMsgFlag');
    ...
    var deleteGroupMember = function () {
        ...
        webim.deleteGroupMember(
                ...
                function (resp) {
                    ...
                    webim.Log.info('deleteGroupMember');
...
//初始化我的加群申请表格
    function initGetApplyJoinGroupPendency(data) {
        ...
        webim.Log.info('initGetApplyJoinGroupPendency');
    ...
    var handleApplyJoinGroupPendency = function () {
        ...
        webim.handleApplyJoinGroupPendency(
                ...
                function (resp) {
                    ...
                    webim.Log.info('handleApplyJoinGroupPendency');
...
//切换好友或群组聊天对象
    function onSelSess(sess_type, to_id) {
        selToID = to_id.toString();//joshua:后为加的，未去理会之前的逻辑
...
//向上翻页，获取更早的好友历史消息
    ...
    var getLastGroupHistoryMsgs = function (cbOk) {
        ...
        getGroupInfo(selToID, function (resp) {
            ...
            $rootScope.groups[selToID] = resp.GroupInfo[0]; //修改当前群信息
...
//     <script type="text/javascript" src="js/msg/send_custom_group_notify_msg.js"></script>
...
//监听 申请加群 系统消息
    function onApplyJoinGroupRequestNotify(notify) {
        ...
        webim.Log.info('onApplyJoinGroupRequestNotify');
...
//增加一条群组系统消息
    function addGroupSystemMsg(type, typeCh, group_id, group_name, msg_content, msg_time) {
        ...
        webim.Log.info('addGroupSystemMsg');
...
//初始化我的好友系统消息表格
    function initGetMyFriendSystemMsgs(data) {
        ...
        webim.Log.info('initGetMyFriendSystemMsgs');
...
//增加一条好友系统消息
    function addFriendSystemMsg(type, typeCh, msgContent) {
        ...
        webim.Log.info('addFriendSystemMsg');
...
//初始化我的资料系统消息表格
    function initGetMyProfileSystemMsgs(data) {
        ...
        webim.Log.info('initGetMyProfileSystemMsgs');
...
//增加一条资料系统消息
    function addProfileSystemMsg(type, typeCh, msgContent) {
        ...
        webim.Log.info('addProfileSystemMsg');
