...
//    };
    ...
    this.select_session = function (sel_type, sel_to_id) {
        selType = sel_type;
        selToID = sel_to_id;
    ...
    //joshua 发送群消息
    this.send_group_message = function (toAccount, msgtosend) {
        sendGroupMsg(toAccount, msgtosend);
    };

    //joshua 获取好友
    this.get_my_friend = function () {
        getMyFriend();
    };

    //joshua 获取好友
    this.get_my_group = function () {
        getMyGroup();
    };
...
//聊天页面增加一条消息
    function addMsg(msg, prepend) {
        var isSelfSend, fromAccount, fromAccountNick, fromAccountImage, sessType, skey, subType;
        //console.log(msg);
        skey = msg.getSession()._impl.skey;

        if (!$rootScope.sessions[skey]) {
            $rootScope.sessions[skey] = {};
        }

        if (!$rootScope.sessions[skey].msgs) {
            $rootScope.sessions[skey].msgs = {};
        }
        $rootScope.sessions[skey].msgs[msg.seq] = msg;
        //获取会话类型，目前只支持群聊
        ...
        if (isSelfSend) {//如果是自己发的消息
            ...
            var key = webim.SESSION_TYPE.C2C + fromAccount;
        ...
        webim.getAllFriend(
                ...
                function (resp) {
                    ...
                    if (resp.FriendNum > 0) {
                        ...
                        for (var i = 0; i < count; i++) {
                            var friend_account = friends[i].Info_Account;
                            ...
                            $rootScope.friends[friends[i].Info_Account] = {
                                'account': friend_account,
                                'name': webim.Tool.formatText2Html(friend_name),
                                'gender': gender
                            };
                            var key = webim.SESSION_TYPE.C2C + friend_account;
                            infoMap[key] = {
                                'account': friend_account,
                                'name': friend_name,
                                'image': friend_image
    ...
    function sendC2cMsg() {
        ...
        webim.sendMsg(msg, function (resp) {
            ...
            if (!selToID) {//没有聊天会话
                ...
                selToID = toAccount;
                selSess = sess;
                addSess(selType, toAccount, toAccount, friendHeadUrl, 0, '');
                ...
                if (selToID == toAccount) {//聊天对象不变
                    ...
                    if (!tempSessDiv) {//不存在这个会话
                        addSess(webim.SESSION_TYPE.C2C, toAccount, toAccount, friendHeadUrl, 0, ''); //增加一个会话
                    }
                ...
                function (resp) {
                    ...
                    if (resp.SessionItem && resp.SessionItem.length > 0) {
                        ...
                        for (var i in resp.SessionItem) {
                            ...
                            if (type == webim.RECENT_CONTACT_TYPE.C2C) {//私聊
                                ...
                                if (!senderNick) {//如果没有设置群成员名片
                                    ...
                                    if (!senderNick) {//如果接口没有返回昵称或者没有设置群昵称，从infoMap获取昵称
                                        var key = webim.SESSION_TYPE.C2C + senderId;
                                        var c2cInfo = infoMap[key];
                            ...
                            tempSess = tempSessMap[sessType + sessionId];
                            if (!tempSess) {//先判断是否存在（用于去重），不存在增加一个
                                tempSessMap[sessType + sessionId] = true;
                                data.push({
                    ...
                    if (resp.SessionItem && resp.SessionItem.length > 0) {//如果存在最近会话记录
                        ...
                        for (var i in resp.SessionItem) {
                            ...
                            if (!tempSess) {//先判断是否存在（用于去重），不存在增加一个
                                ...
                                addSess(sessType, webim.Tool.formatText2Html(sessionId), webim.Tool.formatText2Html(sessionNick), sessionImage, item.UnreadMsgCount, item.MsgShow);
                            }
        ...
        webim.getJoinedGroupListHigh(
                ...
                function (resp) {
                    ...
                    for (var i = 0; i < resp.GroupIdList.length; i++) {
                        ...
                        if (resp.GroupIdList[i].GroupId.indexOf("@TGS#") == -1) {
                            $rootScope.groups[resp.GroupIdList[i].GroupId] = resp.GroupIdList[i];
                        }
...
//从我的群组列表中发群消息
    function sendGroupMsg(toAccount, msgtosend) {
...
//        msgtosend = $("#sgm_content").val();
        ...
        webim.sendMsg(msg, function (resp) {
            ...
            if (!selToID) {//没有聊天会话
                ...
                selToID = toAccount;
                selSess = sess;
                addSess(selType, toAccount, toName, groupHeadUrl, 0, '');
                setSelSessStyleOn(toAccount);
                //群聊时，长轮询接口会返回自己发的消息
                //addMsg(msg);
            } else {//有聊天会话
                if (selToID == toAccount) {//聊天对象不变
                    ...
                    var tempSessDiv = document.getElementById("sessDiv_" + toAccount);
                    if (!tempSessDiv) {//不存在这个会话
                        addSess(webim.SESSION_TYPE.GROUP, toAccount, toName, groupHeadUrl, 0, ''); //增加一个会话
                    }
                ...
                function (resp) {
                    ...
                    if (resp.GroupIdList && resp.GroupIdList.length) {
                        for (var i = 0; i < resp.GroupIdList.length; i++) {
                            ...
                            var key = webim.SESSION_TYPE.GROUP + resp.GroupIdList[i].GroupId;
                            if (group_id.indexOf("@TGS#") == -1) {
                                $rootScope.groups[resp.GroupIdList[i].GroupId] = resp.GroupIdList[i];
                                infoMap[key] = {
                                    'GroupId': group_id,
                                    'Name': group_name,
...
//更新最近会话的未读消息数
    function updateSessDiv(sess_type, to_id, name, unread_msg_count) {
        if (unread_msg_count > 0) {
            ...
            $rootScope.sessions[sess_type + to_id].UnreadMsgCount = unread_msg_count;

        }
    }

//新增一条最近会话
    function addSess(sess_type, to_id, name, face_url, unread_msg_count, msgshow, addPositonType) {
        $rootScope.sessions[sess_type + to_id] = {
            SessionType: sess_type, //会话类型
            SessionId: to_id, //会话对象id，好友id或者群id
            SessionNick: name, //会话昵称，好友昵称或群名称
            SessionImage: face_url, //会话头像，好友头像或者群头像
            UnreadMsgCount: unread_msg_count, //未读消息数,私聊时需要通过webim.syncMsgs(initUnreadMsgCount)获取,参考后面的demo，群聊时不需要。
            MsgShow: msgshow//消息内容,文本消息为原内容，表情消息为[表情],其他类型消息以此类推
        }
//        $rootScope.sessions[session_key]={
//            
//        }
...
//获取最新的群历史消息,用于切换群组聊天时，重新拉取群组的聊天消息
    var getLastGroupHistoryMsgs = function (cbOk) {
        ...
        getGroupInfo(selToID, function (resp) {
            ...
            $rootScope.groups[resp.GroupInfo[0].GroupId] = resp.GroupInfo[0];
            //拉取最新的群历史消息
            var options = {
                'GroupId': selToID,
            ...
            selSess = null;
            webim.MsgStore.delSessByTypeId(selType, selToID);
            //recentSessMap[webim.SESSION_TYPE.GROUP + selToID].MsgGroupReadedSeq = resp.GroupInfo[0].MsgSeq;
            webim.syncGroupMsgs(
                    options,
                    function (msgList) {
                        if (msgList.length == 0) {
...
//totalSize-图片总字节数
    function onProgressCallBack(loadedSize, totalSize) {
        ...
        $rootScope.message_form_upload_bar = (loadedSize / totalSize) * 100;
        //joshua 未做回调进度
        //var progress = document.getElementById('upd_progress'); //上传图片进度条
    ...
    function onMsgNotify(newMsgList) {
        ...
        for (var j in newMsgList) {//遍历新消息
            ...
            if (!selToID) {//没有聊天对象
                ...
                addSess(selType, selToID, newMsg.getSession().name(), headUrl, 0, ''); //新增一个对象
...
//告诉你那个类型的那个群组已读消息的情况
    function onReadedSyncGroupNotify(notify) {
        var seq = notify.MsgReadedSeq;
        //更新当前的seq
        var currentUnRead = recentSessMap[webim.SESSION_TYPE.GROUP + notify.GroupId].MsgGroupReadedSeq;
